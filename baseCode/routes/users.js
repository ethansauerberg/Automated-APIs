...
module.exports=(router, app)=>{
    ...
    router.post('/v1/users', (req,res)=>{
      ...
      if(inputErrorDoc !== null){
        ...
        if(constants.validatePassword(password) == false){
          ...
          MongoOperations.find({email: email}, constants.usersDb, (findErrorDoc, findReturnDoc)=>{
            if(findErrorDoc !== null){
              ...
              if(findReturnDoc.data.length != 0){
                ...
                MongoOperations.insertOne(userDoc, constants.usersDb, (insertErrorDoc, insertReturnDoc)=>{
    ...
    //     }
    //     else {
    //       MongoOperations.updateOne(verifyId, {$set: toUpdate}, {upsert: false}, constants.usersDb, (updateOneErrorDoc, updateOneReturnDoc)=>{
    ...
    router.delete('/v1/users/:email', (req, res)=>{
      ...
      Authorizer.verifyUser(email, password, (verifyErrorDoc, verifyUserDoc, verifyId)=>{
        if(verifyErrorDoc !== null){
          ...
          MongoOperations.deleteOne(verifyId, constants.usersDb, (deleteOneErrorDoc, deleteOneReturnDoc)=>{
